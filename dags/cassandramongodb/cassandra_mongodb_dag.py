...
from cassandramongodb.includes.check_mongodb import check_mongodb_main
from cassandramongodb.includes.kafka_producer import kafka_producer_main
from cassandramongodb.includes.check_cassandra import check_cassandra_main
from cassandramongodb.includes.kafka_create_topic import kafka_create_topic_main
from cassandramongodb.includes.kafka_consumer_mongodb import kafka_consumer_mongodb_main
from cassandramongodb.includes.kafka_consumer_cassandra import kafka_consumer_cassandra_main
...
def decide_branch(**context):
    ...
    print(f"Branch result: {result}")  # optional debug
    return result

otp_cassandra = "123456" 
otp_mongodb = "789012"
email_cassandra = "vramschennai@gmail.com"
email_mongodb = "vramschennai@gmail.com"
...
with DAG('airflow_kafka_cassandra_mongodb', default_args=default_args, schedule_interval='@daily', catchup=False) as dag:
    ...
    topic_failed = DummyOperator(task_id="topic_failed")
    
    join_branch = DummyOperator(task_id='join_branch', trigger_rule='none_failed_min_one_success')  
    
    send_email_cassandra = EmailOperator(
        task_id='send_email_cassandra',
        to="vramschennai@gmail.com",
    ...
    create_new_topic >> [topic_created, topic_already_exists, topic_failed] >> join_branch >> kafka_producer
